...
-KGC Key Generation
1. “./BB_KGC_Setup” generates 8 parameters and 1 master key in folder ‘new_param’

2. move all 9 parameters and master key to folder ’My_param’


- Issuing Key from KGC
1. Input ID to ‘Input_Domain_name.txt’ and move it to EXPORT, execute ‘./EXPORT_ID’ (KGC should running ‘./IMPORT’)

2. 
KGC에서 받은 Input_Domain_name.txt파일(IMPORT 폴더에 있음)을 KGC 폴더로 이동후 BB_KGC_Keygen실행, 새로운 키들이 new_key_level_1에 저장됨
